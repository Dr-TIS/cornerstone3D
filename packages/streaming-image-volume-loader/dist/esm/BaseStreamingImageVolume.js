import { Enums, eventTarget, metaData, imageLoadPoolManager, triggerEvent, ImageVolume, cache, imageLoader, utilities as csUtils, } from '@cornerstonejs/core';
        ...
        this.getImageIdsRequests = (imageIds, scalarData, priority) => {
            ...
            const requests = imageIds.map((imageId, frameIndex) => {
                ...
                const callLoadImage = (imageId, imageIdIndex, options) => {
                    return imageLoader.loadImage(imageId, options).then((image) => {
