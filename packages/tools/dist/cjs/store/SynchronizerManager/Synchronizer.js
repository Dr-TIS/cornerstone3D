...
class Synchronizer {
    constructor(synchronizerId, eventName, eventHandler, options) {
        ...
        this._onEvent = (evt) => {
            ...
            const enabledElement = (0, core_1.getEnabledElement)(evt.currentTarget);
            if (!enabledElement) {
                return;
            }
    ...
    addSource(viewportInfo) {
        ...
        const { element } = (0, core_1.getRenderingEngine)(renderingEngineId).getViewport(viewportId);
    ...
    _updateDisableHandlers() {
        ...
        viewports.forEach(function (vUid) {
            const renderingEngine = (0, core_1.getRenderingEngine)(vUid.renderingEngineId).getViewport(vUid.viewportId);
            if (!renderingEngine) {
                return;
            }
            const { element } = renderingEngine;
            element.removeEventListener(core_1.Enums.Events.ELEMENT_DISABLED, disableHandler);
            element.addEventListener(core_1.Enums.Events.ELEMENT_DISABLED, disableHandler);
        });
    }
}
...
function _getViewportElement(vp) {
    const renderingEngine = (0, core_1.getRenderingEngine)(vp.renderingEngineId);
    if (!renderingEngine) {
